...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:math="http://exslt.org/math">
 ...
 <report>
  ...
  <test>
    ...
    <directory>cbn-pbe0-fast</directory>
  </test>
  ...
  <test>
    ...
    <description>passes if the CBM is the same as in reference:
     <xsl:value-of select="document('eigval.xml')/eigval/kpt[3]/state[6]/@eigenvalue"/> vs. <xsl:value-of select="document('eigval.ref.xml')/eigval/kpt[3]/state[6]/@eigenvalue"/>
    </description>
    <directory>cbn-pbe0-fast</directory>
  </test>
